...
public class TrigraphAnalyzer 
{
	...
	private TrigraphPoly[][][] theTris;
	private TrigraphPoly[] mostFrequent;
							...
							int lastMono)
	{
		...
		theTris = new TrigraphPoly[LetterCounterPoly.NUM_LETTERS][LetterCounterPoly.NUM_LETTERS][LetterCounterPoly.NUM_LETTERS];
		// fill theTris
		for (int row = 0; row < theTris.length; row++)
			for (int col = 0; col < theTris[0].length; col++)
				for (int depth = 0; depth < theTris[0][0].length; depth++)
				{
					theTris[row][col][depth] = new TrigraphPoly(row, col, depth);
				}
		...
		mostFrequent = new TrigraphPoly[LetterCounterPoly.TRIS_TO_OUT];
	...
	private TrigraphPoly getHighestUnlistedTrigraph()
	{
		...
		for (int row = 0; row < theTris.length; row++)
			for (int col = 0; col < theTris[0].length; col++)
				...
				{
					TrigraphPoly tri = theTris[row][col][third];
