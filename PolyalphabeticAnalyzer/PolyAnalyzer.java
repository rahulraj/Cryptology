...
public class PolyAnalyzer 
{
	...
	{
		// If modifying the code to use the automatedDecipher() method, use this constructor
		cipher = cip.toUpperCase();
		freqs = freqOut;
		plainOut = decOut;
	...
	{
		// deciphers text without user input, does not output frequency data
		// does not have all the features that the user input method does
		// if you wish to decipher the text automatically, modify the client to use this method
		// instead of decipher(). Also, use the constructor with theNumAlphs as a parameter
		int friedNum;
		if (numAlphs != 0)
		{
			friedNum = numAlphs;
	...
	{
		LetterCounterPoly counter = new LetterCounterPoly(cipher);
	...
	private int getNumAlphabets()
	{
		...
		//System.out.println("The Index of Coincidence of this text is: " + indexCoincidence);
	...
	private void solveSubCipher(int numAlphabets) throws IOException
	{
		...
		comp.analyzeWithDepth(); 
		
		String decip = comp.getDeciphered();
		int numSolved = 0;
		for (int index = 0; index < decip.length(); index++)
		{
			if ((decip.charAt(index) != '?') && (decip.charAt(index) != ' '))
			{
				numSolved++;
			}
		}
		System.out.println(numSolved + " letters solved.");
		System.out.println("out of: " + decip.length());
