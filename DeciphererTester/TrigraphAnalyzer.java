...
public class TrigraphAnalyzer 
{
							...
							int lastMono)
	{
		...
		/*if (lastMono == -2)
		{
			// this is the code if there are only two digraphs total
			// therefore first and third are the same monoalphabet
			StringBuilder s = new StringBuilder("");
			s.append(thirdMono.substring(1, thirdMono.length()));
			s.append(" ");
			thirdMono = s.toString();
	...
	public void solveY()
	{
		...
		for (int loc = 0; loc < theTris.length; loc++)
		{
			...
			} catch (ArrayIndexOutOfBoundsException e)
			{
				...
				System.err.println(oCip - 65);
				System.err.println(uCip);
				System.err.println(uCip - 65);
